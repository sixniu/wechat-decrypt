...
DEFAULT_SERVICE_CONFIG: dict[str, Any] = {
    "_comment": "services 目录专用配置。JSON 不支持 // 注释，所以用 _xxx_comment 字段说明每个配置项。",
    "mysql": {
        "_comment": "MySQL 数据库连接配置，用于剧本杀拼本消息入库。",
        "host": "127.0.0.1",
        "_host_comment": "MySQL 主机地址，例如 127.0.0.1 或云数据库 IP。",
        "port": 3306,
        "_port_comment": "MySQL 端口，默认 3306。",
        "user": "",
        "_user_comment": "MySQL 用户名。",
        "password": "",
        "_password_comment": "MySQL 密码。",
        "database": "",
        "_database_comment": "MySQL 数据库名。",
        "charset": "utf8mb4",
        "_charset_comment": "数据库字符集，建议 utf8mb4，支持中文和 emoji。",
    },
    "services": {
        "_comment": "各业务服务配置。",
        "jubensha_booking": {
            ...
            "monitored_chatroom_ids": [
                ...
                {"id": "46000926626@chatroom", "name": "剧本杀31群"},
            ...
            "_booking_table_comment": "保存结构化拼本预约数据的 MySQL 表名。",
            "poster_sender": {
            ...
            "free_discount_notice_poller": {
                "_comment": "从服务器轮询免单拼本通知任务。本地微信可用时启用，target_chats 为空时不启动。",
                "enabled": False,
                "_enabled_comment": "是否启用服务器免单通知任务轮询。true=启用，false=关闭。",
                "api_base_url": "https://www.shisan.ink/api/booking/free-notices",
                "_api_base_url_comment": "script-kill-api 的免单通知内部接口基础地址。",
                "token": "",
                "_token_comment": "内部接口 token，需要和服务器 FREE_DISCOUNT_NOTICE_TOKEN 保持一致。",
                "interval_seconds": 10,
                "_interval_seconds_comment": "轮询间隔秒数。空列表响应很小，建议 5-15 秒。",
                "limit": 10,
                "_limit_comment": "每次最多领取的免单通知任务数量。",
                "timeout": 10,
...
def load_service_config_strict() -> dict[str, Any]:
    """读取配置，配置格式错误时直接抛出异常。"""
    cfg: dict[str, Any] = {}
    if os.path.exists(SERVICE_CONFIG_FILE):
        with open(SERVICE_CONFIG_FILE, encoding="utf-8") as file:
